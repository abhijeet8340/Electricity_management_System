...
public class login extends JFrame implements ActionListener {
    ...
    login() {
        ...
        setLocation(600, 400);
        setVisible(true);
        connection = new conn(); //create connection object
    }
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == b1) {
            try {
                String a = tf1.getText();
                String b = new String(pf2.getPassword()); // Get password as char[]

                String q = "select * from login where username = ? and password = ?"; // Prepared statement
                PreparedStatement preparedStatement = connection.c.prepareStatement(q);
